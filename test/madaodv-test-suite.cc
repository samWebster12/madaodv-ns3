...
{
    TypeHeaderTest()
        : TestCase("MADAODV TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(MADAODVTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), MADAODVTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(MADAODVTYPE_RREP);
 ...
 * \ingroup madaodv-test
 *
 * \brief Unit test for MADAODV routing table entry
 */
...
struct MadaodvRqueueTest : public TestCase
{
    MadaodvRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
MadaodvRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&MadaodvRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&MadaodvRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &MadaodvRqueueTest::CheckTimeout, this);
...
void
MadaodvRqueueTest::CheckSizeLimit()
{
...
void
MadaodvRqueueTest::CheckTimeout()
{
 ...
 * \brief Unit test for MADAODV routing table entry
 */
struct MadaodvRtableEntryTest : public TestCase
{
    MadaodvRtableEntryTest()
 ...
 * \ingroup madaodv-test
 *
 * \brief Unit test for MADAODV routing table
 */
struct MadaodvRtableTest : public TestCase
{
    MadaodvRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup madaodv-test
 *
 * \brief MADAODV test suite
 */
class MadaodvTestSuite : public TestSuite
{
  public:
    MadaodvTestSuite()
        : TestSuite("routing-madaodv", UNIT)
    {
        ...
        AddTestCase(new MadaodvRqueueTest, TestCase::QUICK);
        AddTestCase(new MadaodvRtableEntryTest, TestCase::QUICK);
        AddTestCase(new MadaodvRtableTest, TestCase::QUICK);
    }
} g_madaodvTestSuite; ///< the test suite
