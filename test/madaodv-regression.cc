 ...
 *
 * \brief MADAODV regression test suite
 */
class MadaodvRegressionTestSuite : public TestSuite
{
  public:
    MadaodvRegressionTestSuite()
        : TestSuite("routing-madaodv-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("madaodv-chain-regression-test"), TestCase::QUICK);
...
} g_madaodvRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("MADAODV chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & MADAODV
    MadaodvHelper madaodv; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(madaodv);
    ...
    streamsUsed += madaodv.AssignStreams(*m_nodes, streamsUsed);
    // MADAODV uses m_size more streams
