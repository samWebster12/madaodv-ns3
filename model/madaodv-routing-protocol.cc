...
{
    static TypeId tid =
        TypeId("ns3::madaodv::RoutingProtocol")
                            ...
                            ErrorCallback ecb)
{
    ...
    // MADAODV is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvMadaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), MADAODV_PORT));
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("MADAODV does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when MADAODV does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // MADAODV needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available MADAODV interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid MADAODV source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by madaodv
    for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin();
         j != m_socketAddresses.end();
         ++j)
    {
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, MADAODV_PORT));
}
...
void
RoutingProtocol::RecvMadaodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("MADAODV node " << this << " received a MADAODV packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("MADAODV message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), MADAODV_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), MADAODV_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, MADAODV_PORT));
}
                                              ...
                                              Ipv4Address origin)
{
    ...
    else
    {
             ...
             ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, MADAODV_PORT));
        }
    }
}
